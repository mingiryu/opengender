...
class Gender(object):
    ...
    def features_list_categorical(self, path="files/names/partial.csv"):
        ...
        with open(path) as csvfile:
            ...
            for row in sexreader:
                ...
                l1 = list(
                    [
                        self.features_int(name)["first_letter"],
    ...
    def features_list_no_categorical(self, path="files/names/partial.csv"):
        ...
        with open(path) as csvfile:
            ...
            for row in sexreader:
                ...
                l1 = list(
                    [
                        self.features_int(name)["count(a)"],
    ...
    def features_list2csv(self, path, categorical="both"):
        if categorical == "categorical":
            ...
            first_line = (
                "first_letter, last_letter, last_letter_a, "
            ...
            first_line = (
                "first_letter, last_letter, vocals, "
            ...
            first_line = (
                "first_letter, last_letter, a, b, c, d, e, "
            ...
            first_line = (
                "first_letter, last_letter, a, b, c, d, e, f, "
    ...
    def name_prob_countries(self, name):
        ...
        pfemales = {
            "es": du.round_and_not_zero_division(int(es["females"]), int(females)),
            "ie": du.round_and_not_zero_division(int(ie["females"]), int(females)),
            "is": du.round_and_not_zero_division(int(isl["females"]), int(females)),
            "uy": du.round_and_not_zero_division(int(uy["females"]), int(females)),
            "gb": du.round_and_not_zero_division(int(gb["females"]), int(females)),
            "us": du.round_and_not_zero_division(int(us["females"]), int(females)),
            "nz": du.round_and_not_zero_division(int(nz["females"]), int(females)),
            "ca": du.round_and_not_zero_division(int(ca["females"]), int(females)),
            "fi": du.round_and_not_zero_division(int(fi["females"]), int(females)),
            "au": du.round_and_not_zero_division(int(au["females"]), int(females)),
            "pt": du.round_and_not_zero_division(int(pt["females"]), int(females)),
        }

        pmales = {
            "es": du.round_and_not_zero_division(int(es["males"]), int(males)),
            "ie": du.round_and_not_zero_division(int(ie["males"]), int(males)),
            "is": du.round_and_not_zero_division(int(isl["males"]), int(males)),
            "uy": du.round_and_not_zero_division(int(uy["males"]), int(males)),
            "gb": du.round_and_not_zero_division(int(gb["males"]), int(males)),
            "us": du.round_and_not_zero_division(int(us["males"]), int(males)),
            "nz": du.round_and_not_zero_division(int(nz["males"]), int(males)),
            "ca": du.round_and_not_zero_division(int(ca["males"]), int(males)),
            "fi": du.round_and_not_zero_division(int(fi["males"]), int(males)),
            "au": du.round_and_not_zero_division(int(au["males"]), int(males)),
            "pt": du.round_and_not_zero_division(int(pt["males"]), int(males)),
        }
    ...
    def pretty_gg_list(self, guessf, testf, *args, **kwargs):
        ...
        if (os.path.isfile(guessf)) and (os.path.isfile(testf)):
            if du.is_json(guessf):
                ...
                if du.is_json(testf):
                    ...
                    testnames = self.csv2names(path=testf, name_position=name_test_row)
                    if len(guessnames) == len(testnames):
                        print("########################## " + api + "!!")
                        print("Gender Guess (guess list): " + str(gl))
                        print("Gender Test   (true list): " + str(tl))
                        ...
                        difflen = True
                        v = self.first_uneq_json_and_csv_in_names(
                            jsonf=guessf, csvf=testf
                        )

            elif du.is_csv(guessf):
                gl = self.csv2gender_list(
                    path=guessf,
                    binary=True,
                ...
                guessnames = self.csv2names(path=guessf, name_position=name_guess_row)
                if du.is_json(testf):
                    tl = self.json2gender_list(jsonf=testf, binary=True)
                    testnames = self.json2names(jsonf=testf, surnames=False)
                    if len(guessnames) == len(testnames):
                    ...
                    if len(guessnames) == len(testnames):
                        ...
                        difflen = True
                        v = self.first_uneq_csv_and_csv_in_names(
                            csv1=guessf, csv2=testf
                        )
        ...
        if difflen:
            ...
            print("%s names in test file" % len(testnames))
            print("%s names in guessed file" % len(guessnames))
            print("The unmatched names starts in %s and the name is %s" % (v[1], v[0]))
            print("Names in test file: %s:" % testnames)
            print("Names in guessed file: %s:" % guessnames)
        return 1
    ...
    def pretty_cm(self, path, jsonf, *args, **kwargs):
        ...
        if os.path.isfile(jsonf):
            self.print_confusion_matrix_gender(
                path=path, dimensions=dimensions, jsonf=jsonf, reverse=reverse
            )
        elif args.jsondownloaded == "":
            self.print_confusion_matrix_gender(
                path=path, dimensions=dimensions, reverse=reverse
            )
        else:
    ...
    def json_eq_csv_in_names(self, jsonf="", path="", *args, **kwargs):
        ...
        while maxi > i:
            if json_lower[i] == csv_lower[i]:
                count = count + 1
            i = i + 1
        boolean = (len(json_lower) == len(csv_lower)) and (
            (len(json_lower) - 1) == count
        )
        return boolean
    ...
    def first_uneq_json_and_csv_in_names(self, jsonf="", path="", *args, **kwargs):
        ...
        maxi_csv = len(csv) - 1
        while (
            (i < maxi_json) and (i < maxi_csv) and (json[i].lower() == csv[i].lower())
        ):
            i = i + 1
        ret = json[i].lower()
    ...
    def first_uneq_json_and_json_in_names(self, json1="", json2="", *args, **kwargs):
        header = kwargs.get("header", True)
        ...
        while (
            (i < maxi_json1)
            and (i < maxi_json2)
            and (json1[i].lower() == json2[i].lower())
        ):
            i = i + 1
        ret = json1[i].lower()
    ...
    def first_uneq_csv_and_csv_in_names(self, csv1="", csv2="", *args, **kwargs):
        header = kwargs.get("header", True)
        ...
        while (
            (i < maxi_csv1) and (i < maxi_csv2) and (csv1[i].lower() == csv2[i].lower())
        ):
            i = i + 1
        ret = csv1[i].lower()
